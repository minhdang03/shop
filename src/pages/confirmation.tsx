...
export default function Confirmation() {
    ...
    useEffect(() => {
        ...
        if (savedOrderInfo) {
            ...
            const expirationTime = orderData.expiry;  // Lấy thời gian hết hạn đã lưu

            if (new Date().getTime() > expirationTime) {
                // Nếu đã hết hạn, xóa dữ liệu và điều hướng về trang chủ
                localStorage.removeItem('lastOrderInfo');
                navigate('/');
            } else {
                setOrderInfo(orderData);  // Nếu còn hiệu lực, lưu dữ liệu
            }
        } else {
            navigate('/');
        }
