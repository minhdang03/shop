...
function ProductMen({ limit = 0, updateTitle = false }: ProductMenProps) {
  useDocumentTitle(updateTitle ? 'Nước hoa nam' : '');
  ...
  const { data: products = [], isLoading, error } = useQuery({
    queryKey: ['men-products', limit],
    queryFn: async () => {
      const response = await fetch(
        `${API_URL}/api/user/products?category=676057995957fd2f56673f7e${limit ? `&limit=${limit}` : ''}`
      );
...
export default memo(ProductMen);
